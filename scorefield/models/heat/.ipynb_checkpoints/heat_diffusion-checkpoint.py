...
class HeatDiffusion(object):
    def __init__(self, image_size, u0=1, noise_steps=500, heat_steps=200000, alpha=1.0, beta=10.0, precision='single', device='cuda'):
    ...
    def compute_K(self, u):
        ...
        dt = torch.stack([torch.tensor(distance_transform_edt(mask.cpu().numpy())) for mask in ~obstacle_with_edges])
        dt = dt.to(u.device).float()
        decay = torch.exp(-dt/self.beta)
    ...
    def gaussian_filter(self, input_tensor, kernel_size=5, std=1.0):
        ...
        # gaussian_filter = torch.outer(kernel, kernel).unsqueeze(0).unsqueeze(0)  # (1, 1, kernel_size, kernel_size)
    ...
    def revise_obstacle_masks(self, batch_size, obstacle_masks):
        ...
        # else:
        #     obstacle_masks = kornia.morphology.dilation(obstacle_masks.unsqueeze(1), torch.ones(5,5, dtype=obstacle_masks.dtype, device=obstacle_masks.device)).squeeze(1)
...
#             laplacian[:, 1:-1, 1:-1] = top + bottom + left + right - valid_neighbors * u[:, 1:-1, 1:-1]
    ...
    def sample_from_heat(self, ut):
        B = ut.size(0)
        flattened_dist = ut.flatten().view(B, -1)
        sampled_indices = torch.multinomial(flattened_dist, 1).squeeze()
    ...
    def score(self, u, x):
        ...
        filtered_score = self.gaussian_filter(score_field)
        
        batch_indices = torch.arange(u.shape[0])
        x_indices = x[:, 0]
        y_indices = x[:, 1]
        score = filtered_score[batch_indices, x_indices, y_indices, :]
        score = clip_batch_vectors(score, 0.01)
    ...
    def forward_diffusion(self, time_steps, heat_sources, obstacle_masks=None):
        ...
        return self.score(ut_batch, x_t), self.convert_space(x_t, 'norm')
        return ut_batch
